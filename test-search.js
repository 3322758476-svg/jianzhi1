...
async function testSearch() {
  ...
  const { data: frontendJobs, error: error1 } = await supabase
    ...
    .select('*')
    .eq('status', 'active')
    .or('title.ilike.%前端开发%,description.ilike.%前端开发%,skills_required.ilike.%前端开发%');
  
  if (error1) console.error('搜索失败:', error1);
  else console.log('找到', frontendJobs?.length || 0, '个岗位');
  ...
  const { data: mediaJobs, error: error2 } = await supabase
    ...
    .select('*')
    .eq('status', 'active')
    .or('title.ilike.%新媒体%,description.ilike.%新媒体%,skills_required.ilike.%新媒体%');
  
  if (error2) console.error('搜索失败:', error2);
  else console.log('找到', mediaJobs?.length || 0, '个岗位');
  ...
  const { data: dataJobs, error: error3 } = await supabase
    ...
    .select('*')
    .eq('status', 'active')
    .or('title.ilike.%数据分析%,description.ilike.%数据分析%,skills_required.ilike.%数据分析%');
  
  if (error3) console.error('搜索失败:', error3);
  else console.log('找到', dataJobs?.length || 0, '个岗位');
  ...
  const { data: uiJobs, error: error4 } = await supabase
    ...
    .select('*')
    .eq('status', 'active')
    .or('title.ilike.%UI设计%,description.ilike.%UI设计%,skills_required.ilike.%UI设计%');
  
  if (error4) console.error('搜索失败:', error4);
  else console.log('找到', uiJobs?.length || 0, '个岗位');
